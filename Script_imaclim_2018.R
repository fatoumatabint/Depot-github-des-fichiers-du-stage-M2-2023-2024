...
baci_2018 <- read.csv("BACI_HS02_Y2018_V202401b.csv")
...
baci_2018 <- baci_2018 %>%rename(année = t, exportation = i, importation = j, produit = k, valeur_thousands_current_usd = v, quantité_metric_tons = q )
hs_gtap <- hs_gtap %>% rename(produit = HS.2022.Product.Code)

# Jointure et nettoyage des données ----

baci_gtap_full <- full_join(baci_2018, hs_gtap, join_by("produit")) %>%
  na.omit() %>%
...
write_xlsx(table_exports_imports,"table_exports_imports_2018.xlsx")
...
balance_commerciale_2018 <- final_exports_imports %>%
  mutate(Balance = total_exports - total_imports) %>%
  select(regions_codes, Balance)

# Afficher le résultat

print(balance_commerciale_2018)

## Calcul du deséquilibre mondiale 

déséquilibre_mondial_2018 <- c(sum(balance_commerciale_2018$Balance))
...
plot <- ggplot(data_totals, aes(x = imaclim_sector, y = pourcentage, fill = imaclim_sector))  +
  ...
  labs(title = "Part du Commerce par Secteur en 2018",
       x = "Secteur Imaclim",
       y = "Part du commerce",
       fill = "Secteur_Imaclim",
       caption = "Source: Données BACI, 2018") +
  theme_minimal() +
  theme(
...
balance_commerciale_pourcentage <- balance_commerciale_2018 %>%
  group_by(regions_codes) %>% 
...
plot_region <- ggplot(balance_commerciale_pourcentage, aes(x = regions_codes, y = pourcentage, fill = regions_codes)) +
  ...
  labs(title = "Part du Commerce par région en 2018",
       x = "Régions",
       y = "Part du commerce",
       fill = "Région Imaclim", 
       caption = "Source: Données BACI, 2018") +
...
facet <- ggplot(table_exports_imports_longer, aes(x = imaclim_sector, y = pourcentage, fill = imaclim_sector)) +
  ...
  labs(
    title = "Part du Commerce par Région et par Secteur en 2018",
    x = "Région",
